...
Vocabulario::volcarAMemoria (PtrBuffer ptrInicioDatos) const
{
	...
	// strncpy_s: destination, size of destination, source, number of chars to be copied
       // strncpy(palabraTmp, (*it).second.palabraCorta,5);
		strncpy_s(palabraTmp,6, (*it).second.palabraCorta, 5);
    ...
    while (actual != NULL)
    {
        if ((unsigned int)actual->codigo != 1 && (unsigned int)actual->codigo != 255)
        {
            ...
            os << actual->palabra(flags)
               ...
               << ((flags&VERCPM||flags&CONVERTDSF)&&tipoAStr(actual->tipo)=="CONJUNCTION"?"CONJUGATION":tipoAStr(actual->tipo))
               << endl;
        }
...
Vocabulario::leerDeCodigo (Tokeniser& tokeniser)
{
    ...
    do
    {
        ...
        {
			//printf(llenaespacios5(tokeniser.lvalue().literal).c_str());
			//advised to change by vs:
			// strncpy_s: destination, size of destination, source, number of chars to be copied
			//strncpy(palabra.palabraCorta, llenaespacios5(tokeniser.lvalue().literal).c_str(), 5);
            strncpy_s(palabra.palabraCorta, 6,llenaespacios5(tokeniser.lvalue().literal).c_str(), 5);
