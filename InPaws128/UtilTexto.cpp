        ...
        else
        {
            ...
            switch (texto[i])
            {
                ...
                case '\"':
                    ...
                    break;
                default:
                    if (texto[i] > 0 && isprint(texto[i]) && (int)(unsigned char)texto[i] < 128)
                    {
                        cadenaRet.append(1, texto[i]);
                    }
                    else
...
string
UtilTexto::filtrarParaDSF(const string& texto)
{
    string textoCompacto = compactarTexto(texto);
    string cadenaRet = "";
    int tamlineaactual(0);
    for (size_t i = 0; i < textoCompacto.size(); i++)
    {
        ...
        // dec 2021 - the following end of the next line was commented out
        // have uncommented it and it now excludes udgs from appearing in the dsf file
        //                                             && (unsigned int)textoCompacto[i] <= 0x80
        else if ((unsigned int)textoCompacto[i] >= ' ' && (unsigned int)textoCompacto[i] <= 0x80)
        {
            if (textoCompacto[i] == ' ')
            {
