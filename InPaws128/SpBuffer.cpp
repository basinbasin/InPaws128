...
SpBuffer::writeBufferTAP (const std::string& nombre)
{
	...
	//jm - if full filepath used, use the filename from the final directory
	if (nombre.find("\\") > 0) {
		basename = nombre.substr(nombre.find_last_of("/\\")+1, nombre.find_last_of("."));
	}
    
    // Lo primero, justificamos el nombre a 9 caracteres rellenando con espacios
	//jm use last 9 letters of filepath, not first
	...
	//JM need to replace this so it saves in the right place
	//string ficheroStr = basename + ".TAP";
    string ficheroStr = nombre.substr(0, nombre.find_last_of("."))+".TAP";
	...
	if (bytePeek(PAGE_1_MSGS) != 0) {

		//amend the message volume in the following to indicate there's another page with messages in
		PtrBuffer mainTop = 37632;
		//messages
		
		
		SpByte page1messagenumber = SPBUFFER->bytePeek(528);


		SpByte numMsgs = SPBUFFER->bytePeek(mainTop + OFF_MENSAJES);
		SPBUFFER->bytePoke(mainTop + OFF_128KOTRO, numMsgs - page1messagenumber);
		//zz
		cout << endl;
		cout << "**"  << int(page1messagenumber) << " of the " << int(numMsgs) << " messages were moved to Page 1.**" << endl;
        

		//locations	
        //maintop is gathered by wordpeek contents of 65533/4

        // OFF_128K = 298
        // OFF_NUMLOC = 325
        
        // contains number of locations in standard memory
		SpByte numLocs = SPBUFFER->bytePeek(mainTop + OFF_NUMLOC);
		SPBUFFER->bytePoke(mainTop + OFF_128K, numLocs - 1);
        //printf("numlocs: %d, %d \n", numLocs, mainTop + OFF_128K);

        cout << "**The final location of the " << int(numLocs) << " locations was moved to Page 1.**" << endl;

		//37957 appears to contain total number of locations across all pages
		SPBUFFER->bytePoke(37957, numLocs);

		//37958 appears to contain total number of messages across all pages
		// although looks like code already takes care of this at top of xx
		SPBUFFER->bytePoke(37958, numMsgs);
	...
	if (bytePeek(PAGE_1_MSGS) != 0) {
		...
		_saveTapeBlock(ofstr, nombreReal + "C", 9152, wordPeek(517)-1-40000);
       
		_saveTapeBlock(ofstr, nombreReal + "D", 484, 535);
	}
	...
	PtrBuffer inicio, PtrBuffer final)
{
	...
	header[0] = 3;
	//advised by vs to change this line
	// strncpy: destination, source, number of chars to be copied
	// strncpy_s: destination, size of destination, source, number of chars to be copied
   //strncpy ((char*)header+1, nombre.c_str(), 10);
	strncpy_s((char*)header + 1, 11, nombre.c_str(), 10);
	...
	if (nombre[9] == 'C') {
		...
		header[13] = (SpByte)(inicio + 40000 & 0x00ff);
		header[14] = (SpByte)(inicio + 40000 >> 8);
	}else if(nombre[9] == 'D') {
		
		header[13] = (SpByte)(inicio + 65000 & 0x00ff);
		header[14] = (SpByte)(inicio + 65000 >> 8);
	}
...
SpBuffer::compararMemoria(const std::string& otroFicheroSNA) const
{
	...
	// This function compares with another adventure in SNA
	// Only the addresses occupied by the current adventure
