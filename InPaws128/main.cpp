...
#include "include/prep/Preprocesador.h"
#include "include/UtilTexto.h"
#include "include/Fichero.h"
#include "include/prep/PrepTokeniser.h"
...
bool conversionDesdeSpectrum = false; // Utilizar si se generan .SCE desde aventuras extraidas de spectrum
									//Use if.SCE is generated from adventures extracted from spectrum

int percentmove;
int percentmovearg = false;
...
int main(int argc, char** argv)
{
    ...
    try
    {
        ...
        else if (comando == "cf") // Compile a PC version with DSF output
        {
            int opciones = VERPC + NOCOMPRIMIR + CONVERTDSF;
            if (conversionDesdeSpectrum) opciones += CONVERTSPE;
            compilarADisco(opciones);
            exit(0);
        }
...
void leerParametros(int argc, char** argv)
{
    ...
    if (!strcmp(argv[1],"c") || !strcmp(argv[1],"e") || !strcmp(argv[1],"cp") || !strcmp(argv[1],"cs") ||
        !strcmp(argv[1],"eg") || !strcmp(argv[1],"ec") || !strcmp(argv[1],"cm") || !strcmp(argv[1],"cd") || !strcmp(argv[1], "cf") ||
...
//                conversionDesdeSpectrum = true;
			...
			else if (percentmovearg == true)
			{
				std::stringstream convert(argv[i]); // set up a stringstream variable named convert, initialized with the input from argv[1]

				int myint;
				if (!(convert >> myint)) // do the conversion
					myint = 0; // if conversion fails, set myint to a default value

				//std::cout << "InPaws: Page1 percentage value: " << myint << '\n';
				percentmove = myint;
				//store the percentage in the array holding the spectrum memory
				SPBUFFER->bytePoke(0, percentmove);
			}
            else
            {
                cerr << MSG_SINTAXISINC << endl;
...
void compilarFuente()
{
	...
	printf("InPaws128 v%s.\n",INPAWS_VERSION_128);
	printf("**Based on:\n");	
	cout << "**InPAWS " << INPAWS_VERSION << " Build " << INPAWS_BUILD << endl;
	cout << "**" << INPAWS_COPYRIGHT << endl;
	cout << "Working...\n" << endl;
...
void compilarADisco(uint16_t flags)
{
    ...
    try
    {
        ...
        cout << "InPaws128 " << INPAWS_VERSION_128 << endl;
