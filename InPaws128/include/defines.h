...
namespace Inpaws
{
    ...
    const SpWord PTR_MENSAJES = 65503; //points to the end of the message block, not the beginning
	...
	//The start location to start writing the page 1 messages to:
	const SpWord PAGE_1_MSGS = 9168;

    //The start location to start writing the page 1 location to:
    const SpWord PAGE_1_LOCS = 9163;

    // Desplazamientos respecto al MainTop de los contadores de datos (mensajes,objetos,...)
	// Offsets with respect to the MainTop of the data counters (messages, objects, ...)
	...
	//	Start of 128k page indicators(255 for undefined)
    const SpWord OFF_128K = 298; // Offset for ramtop
    const SpWord OFF_128KOTRO = 284; // For ramtop
    ...
    enum { VERSPECTRUM = 1,   // Versión .TAP para Spectrum
           ...
           CONVERTDSF = 128     };   // Version .DSF from PC
}
