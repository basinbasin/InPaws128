...
// SUMMARY OF THE ALGORITHM THAT THE PREPROCESSOR FOLLOWS
// 1) Read the code without preprocessing the elements of the adventure
// 2) Add, as you read them:
// 2.1) The names of the new entities defined with the dictionary identifier
// 2.2) The codes of the new entities defined with the dictionary number
// 2.2.1) For the latter, add to the assigned pool so that they are not assigned later
// 3) Once ALL the reading of the code without preprocessing is finished, values ??are assigned to the identifiers
// not defined, taking the unassigned from the pool.
// 4) The preprocessed code of each entity is generated, replacing the references to the elements
// of the adventure by the values ??stored in the dictionary.
