                         ...
                         const Diccionario::TDiccionarioInverso* diccionario) const
{
    ...
    // JM May 2020
    // populate a string with the condact to be output so we can 
    // filter it if required
    string outputcondact = condacto->nombre();

   outputcondact = parseDFS(outputcondact, flagsDeFormato);
    ...
    if (condacto->numParams() > 0)
    {
            ...
            condacto->nombre() == "NOUN2")
        {
            ...
            {
            if (flagsDeFormato & CONVERTDSF && outputcondact=="PROCESS") {
                //do something different for process
                //add 3 onto the value!

                _printNombreOValor(os, dato.prm1+3, tipo_prm1(), diccionario);
            }
            else {
                _printNombreOValor(os, dato.prm1, tipo_prm1(), diccionario);
            }
        }
...
string
Condacto::parseDFS(string condact, int flags) const
{
   
    if (flags & CONVERTDSF) {

        // for a full list, see how ANTUR does it at 
        // https://github.com/daad-adventure-writer/antur/blob/master/antur.py

        if (condact == "INVEN") {
            return "SYSMESS 9\n\t\tLISTAT CARRIED\n\t\tSYSMESS 10\n\t\tLISTAT WORN\n\t\tDONE\t\t;InPaws128 replaced INVEN";
        }
        else if (condact == "TURNS") {
            return "SYSMESS 17\n\t\tDPRINT fTurns\n\t\tSYSMESS 18\n\t\tSYSMESS 19\n\t\tSYSMESS 20\t\t;InPaws128 replaced TURNS";
        }
        else if (condact == "SCORE") {
            return "SYSMESS 21\n\t\tPRINT fScore\n\t\tSYSMESS 22\t\t;InPaws128 replaced SCORE";
        }
        else if (condact == "DESC") {
            return "RESTART\t\t;InPaws128 replaced DESC";
        }
        else if (condact == "PROCESS") {
            // deal with process by itself only so we can follow it through code for debugging!
            return condact;
        }
        else if (condact == "PROMPT") {           
            return "SYSMESS";
        }
        else if (condact == "TIMEOUT") {
            return "HASAT TIMEOUT";
        }
        else if (condact == "PARSE") {
            return "PARSE 1";
        }
        else if (condact == "LOAD"|| condact == "SAVE") {
            return "#ifndef \"tape\"\n\t\tX"+condact + " 0\n\t\t#endif\n\t\t#ifdef \"tape\"\n\t\t" + condact + " 0\n\t\t#endif\n\t\tRESTART\t\t;InPaws128 replaced " + condact;
        }
        else {
            return condact;
        }
